...
int main(int argc, char *argv[])
{
    ...
    su3_cdble v_field[VOLUME];
    su3_cdble w_field[VOLUME];

    for (size_t i = 0; i < VOLUME; i++) {
        random_su3_cdble(&u_field[i]);
        random_su3_cdble(&w_field[i]);
        random_su3_cdble(&v_field[i]);
    }
    ...
    usu3xusu3(w_field, w_field, v_field, VOLUME);
    ...
    complex_dble tr = su3_trace(&w_field[idx]);
