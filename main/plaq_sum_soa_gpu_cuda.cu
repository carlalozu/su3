...
/* Each thread handles one lattice site and accumulates contributions from all
 * 6 (mu,nu) plaquette orientations.  With periodic BC (bc=3) every plaquette
 * contributes without weight, matching local_plaq_sum_dble(iw=0).
 *
 * d_fld is a su3_mat_field with device pointers; link (ix,mu) is at flat
 * index mu*volume+ix, identical to the AoS kernel's mu*volume+ix offset.
 * d_iupT[mu*volume+ix] is the forward neighbor of site ix in direction mu.
 */
...
int main(int argc, char *argv[])
{
    ...
    for (int r = 0; r < 3; r++) {
        ...
        plaq_sum_kernel<<<blocks, THREADS>>>(d_total, d_fld, d_iupT, VOLUME);
    }
